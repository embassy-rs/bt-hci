...
impl<'de> ReadHci<'de> for AclPacket<'de> {
    fn read_hci<R: embedded_io::blocking::Read>(
        ...
        let (header, buf) = AclPacketHeader::read_hci(&mut reader, buf)?;
        let data_len = header.data_len();
        if buf.len() < data_len {
            Err(ReadHciError::BufferTooSmall)
        } else {
            let (buf, rest) = buf.split_at_mut(data_len);
            reader.read_exact(buf)?;
            Self::from_header_hci_bytes(header, buf)
                .map(|(x, _)| (x, rest))
                .map_err(Into::into)
        }
    }
    ...
    async fn read_hci_async<R: embedded_io::asynch::Read>(
        ...
        let (header, buf) = AclPacketHeader::read_hci_async(&mut reader, buf).await?;
        let data_len = header.data_len();
        if buf.len() < data_len {
            Err(ReadHciError::BufferTooSmall)
        } else {
...
impl<'de> ReadHci<'de> for SyncPacket<'de> {
    fn read_hci<R: embedded_io::blocking::Read>(
        ...
        let (header, buf) = SyncPacketHeader::read_hci(&mut reader, buf)?;
        let data_len = header.data_len();
        if buf.len() < data_len {
            Err(ReadHciError::BufferTooSmall)
        } else {
    ...
    async fn read_hci_async<R: embedded_io::asynch::Read>(
        ...
        let (header, buf) = SyncPacketHeader::read_hci_async(&mut reader, buf).await?;
        let data_len = header.data_len();
        if buf.len() < data_len {
            Err(ReadHciError::BufferTooSmall)
        } else {
...
impl<'de> ReadHci<'de> for IsoPacket<'de> {
    fn read_hci<R: embedded_io::blocking::Read>(
        ...
        let (header, buf) = IsoPacketHeader::read_hci(&mut reader, buf)?;
        let data_load_len = header.data_load_len();
        if buf.len() < data_load_len {
            Err(ReadHciError::BufferTooSmall)
        } else {
            let (buf, rest) = buf.split_at_mut(data_load_len);
            reader.read_exact(buf)?;
            Self::from_header_hci_bytes(header, buf)
                .map(|(x, _)| (x, rest))
                .map_err(Into::into)
    ...
    async fn read_hci_async<R: embedded_io::asynch::Read>(
        ...
        let (header, buf) = IsoPacketHeader::read_hci_async(&mut reader, buf).await?;
        let data_load_len = header.data_load_len();
        if buf.len() < data_load_len {
            Err(ReadHciError::BufferTooSmall)
        } else {
